...
- **陣営戦の挙動**: 各兵は `{faction, hp, target, atkCd, hitFlash, alive}` を持つ。索敵=思考フレームで最寄りの敵陣ユニット(`AGENT_SIGHT=64`)を `target` に。移動=毎フレーム target 方向へ向き直して前進。**壁は1段差なら登り、それ以上は向きを散らして回り込む＝地形は一切掘らない/壊さない**。戦闘=射程`AGENT_ATK_RANGE=1.7`内で `AGENT_ATK_CD=0.5s` 毎に `AGENT_ATK_DMG=2` を与える(`AGENT_HP=6`＝3発で撃破)。被弾で白フラッシュ(hitFlash)＋軽いノックバック。**核なし＝戦闘は近接melee のみ・地形破壊なし**。戦死時のみ小さな破片パフ(`createBlockParticles`・地形は変えない・throttle付き)。
