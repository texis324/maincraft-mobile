...
- **selftest**: 11e(legion summon+update＋色バッファ検証)・11f(離れた赤×青が**石の足場上で撃ち合い**HP減少/撃破＋曳光弾発生。⚠自然地形だと起伏で埋まる/LOS不通になるので平らな足場を`setBlockData`で敷く)・11g(プレイヤーの爆発で兵が死ぬ)・11h(敵全滅→勝者軍が自動消去・タイマーのリセット/カウント)。`window.__selftest()` で緑確認(現在21項目)。⚠**`castle nuke live==reload` はシード依存で1ボクセル差が出る既存の潜在エッジがある**(diff 0/1がseedで変動・agents変更とは無関係・要別途調査)。
