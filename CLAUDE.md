...
- **性能の土台（数百体でも軽い）**: ①描画＝本体`InstancedMesh`(setColorで陣営色)＋銃`InstancedMesh`(gunMesh・暗いガンメタル)＝計2ドローコール ②曳光弾＝全弾を1つの`THREE.LineSegments`(tracerLines・固定バッファ`TRACER_MAX=512`・1ドローコール) ③思考＝ラウンドロビン(`AGENT_THINK_BUDGET=14`体/frameだけ最寄り敵を索敵) ④`VIEW_DIST*16` 外/奈落のユニットは消滅。**`AGENT_MAX=800`/`AGENT_WAVE=80`(赤40+青40・連打で上限まで増援＝大規模戦争)**。実測 140体で avg0.13ms、**800体で updateAgents avg0.45ms/max3.6ms**(LOSレイ＋曳光弾込み・16ms予算に余裕)。`AGENT_SIGHT=90`(大軍が広く展開しても全員が敵を見つけて収束)。
- **見た目**: 明るめグレーの胴体＋**軍隊っぽいヘルメット(上部の暗いドーム＋ブリム線・`createAgentTexture`)**＋光る目＋**手元に銃(細長い箱・heading方向へ構える)**。tint で陣営色のヘルメット兵に。
- **★陣営戦の挙動＝銃で撃ち合い（ranged・2026-06-20）**: 各兵は `{faction, hp, target, fireCd, hasLos, hitFlash, alive, turnDir}` を持つ。索敵=思考フレームで最寄りの敵陣(`AGENT_SIGHT=90`)を `target` に。**射程`AGENT_FIRE_RANGE=20`に入り射線(LOS)が通れば停止して射撃(engaged)・通らなければ前進して回り込む**。射撃=ヒットスキャン(`AGENT_FIRE_CD=0.6s±`毎・命中率`AGENT_ACCURACY=0.55`・`AGENT_FIRE_DMG=2`/`AGENT_HP=6`＝3発)。命中は曳光弾＋白フラッシュ、外れも曳光弾だけ飛ぶ(撃ち合いが賑やか)。**`voxelRaycast`のLOS判定は「発射CD時だけ」走らせてレイ数を抑制**(in-rangeでも毎フレームは撃たない)。両軍とも射程で停止→約20ブロックの距離で撃ち合う。**核なし・地形破壊なし**。戦死時のみ小さな破片パフ(throttle)。曳光弾は陣営色(赤=橙赤/青=シアン)・`TRACER_LIFE=0.09s`。
...
- **★敵全滅で勝者軍を自動消去（2026-06-20）**: 描画ループで陣営の生存を集計し、**1陣営だけ残ったら(=敵全滅) `AGENT_DESPAWN_DELAY=10`秒後に `clearAgents()`**＝勝者の残党が邪魔にならない(Tench「赤軍が強すぎて残るので・敵がいなくなって10秒で自動で消えるように」)。両陣営そろう／空ならタイマー(`_battleOverTimer`)リセット＝増援召喚で敵が戻れば消えない。ワールドは保ったまま兵だけ消える(再生成はワールドも復元してしまうので不可)。
- **★片陣営だけ召喚＋公平性（2026-06-21）**: 召喚アイテムを3種に＝**SUMMONER(赤青同時・会戦)/SUMMON_RED(106・赤だけ)/SUMMON_BLUE(107・青だけ)**。`summonFaction(faction, onWest)`。**負けてる側だけ増援できる＝リスポーンキル(敵陣のど真ん中に湧いて即死)回避**(Tench「軍を増やしても青がすぐ殺され赤ばっか増える・どちらかだけ増やしたい」)。★**「赤が強い」の正体=コードでなく地形**: 戦闘ロジックは完全対称(平地のリングで召喚すると赤青 約50/50)。勝つのは常に**西側**(red=西/blue=東の既定spawn＋その地形差)。→ **summonLegion は西/東に湧く陣営を毎回ランダム化**して偏りを散らす(単体summonFactionは方角固定＝増援位置を予測可能に)。アイコンは textures.js `summon_red`/`summon_blue`(陣営色の兵＋増援＋バッジ)。
- **★死体が残る（2026-06-21）**: 戦死した兵を**倒れた暗い陣営色のスラブ**として残す(`corpses`/`corpseMesh`・InstancedMesh 1ドローコール・`CORPSE_MAX=500`で古いの捨て)＝戦争っぽさ(Tench要望)。死亡時だけ `addCorpse`→`_corpsesDirty`で作り直し(毎フレームはノーオペ)。**カリング/奈落落下は死体を残さない**(実際に倒された時だけ)。`clearAgents`(手動/自動消去)で死体も消える。
- **★銃声（2026-06-21）**: `audio.js` に `gunshot`(ハイパスノイズの鋭いクラック＋低い芯)。`_agentFire` から**プレイヤー60ブロック以内の発射だけ・グローバル間引き(`_agentShotSndCD`+audio側 throttle 0.03s)**で鳴らす＝大量発射でもスパムしない(Tench「静かすぎる」)。
...
- **selftest**: 11e(legion summon+update＋色バッファ検証)・11f(離れた赤×青が**石の足場上で撃ち合い**HP減少/撃破＋曳光弾発生。⚠自然地形だと起伏で埋まる/LOS不通になるので平らな足場を`setBlockData`で敷く)・11g(プレイヤーの爆発で兵が死ぬ)・11h(敵全滅→勝者軍が自動消去・タイマーのリセット/カウント)・11i(片陣営召喚summonFaction＋戦死で死体が残る/clearで死体も消える)。`window.__selftest()` で緑確認(現在22項目)。⚠**`castle nuke live==reload` はシード依存で1ボクセル差が出る既存の潜在エッジがある**(diff 0/1がseedで変動・agents変更とは無関係・要別途調査)。
- **チューニング余地**: 密集会戦は決着が速い(56体→2秒で十数体)。長引かせたいなら `AGENT_HP↑`/`AGENT_ATK_DMG↓`/`AGENT_ATK_CD↑`。

## 追加機能: プレイヤー武器（ライフル/レールガン）＋クソデカ山＋無音テスト（2026-06-21）

- **🔫 アサルトライフル(`RIFLE=108`)**: 右クリ/C で連射(`RIFLE_DELAY=110`)。`fireRifle`(entities.js)＝`voxelRaycast`で壁までの距離を取り、`rifleHitscan`(agents.js)で**視線上の最初の兵をヒットスキャン**(`RIFLE_DMG=4`/兵HP6=2発)。黄の曳光弾(`addTracerRGB`)＋`gunshot`音＋リコイル。ビューモデル `rifleGunGroup`(scene.js)。将来モブ/巨大モンスター用。
- **⚡ レールガン(`RAILGUN=109`)**: 右クリ/C(`RAILGUN_DELAY=550`)。`fireRailgun`(entities.js)＝視線方向へ `RAIL_RANGE=260`・半径 `RAIL_R=1.6` の**トンネルを一直線に穿つ(重なる球で連続化・`removeBlock`defer→flush)**。岩盤/水は残す。貫通線上の兵も薙ぐ。シアンのビーム(`spawnRailBeam`/`updateRailBeams`・0.35sフェード・main.jsで更新)＋`railgun`音＋画面揺れ。ビューモデル `railgunGunGroup`(発光レール)。**標高117の巨大山を120ブロック貫通を実機確認**。Tench「どんな山でも貫ける設定」→範囲は要望で延長可。
- **★クソデカい山(`TERRAIN_GIANT_AMP=80`)**: `terrainHeightAt` に超低周波ノイズの高い所だけを二乗で鋭く持ち上げる **additive項**を追加(`gp=clamp((giant-0.58)/0.30); +gp*gp*GIANT_AMP`)＝谷は深くせず rare な巨大山が点在。**`maxSurfaceY` に `+TERRAIN_GIANT_AMP` を足すのを忘れない**(最高峰クリップ防止)。実測 標高117の山。城は平地要件(mx-mn≤10)で巨大山の急斜面を避けるので埋没しない(壁86%石を確認)。
- **🔇 無音テスト(`?mute` / `window.__setMute(true)`)**: masterGain を 0 に。Playwright検証は `?selftest&mute` で無音。⚠ヘッドレスは元々audio suspendで鳴らないが、明示mute推奨。
- **selftest 25項目**: 11j(rifle hitscanで兵HP減)・11k(railgunが厚い壁を貫く)・11l(広域走査で巨大山が存在)。+ **11b「castle generates」を単一点→外壁の石比率(>0.6)に堅牢化**(銃眼/門の隙間で単一点が外れる脆さを解消＝シード変動に強く)。⚠`castle nuke live==reload` のシード依存1ボクセル差は依然 別件(要調査)。

## 追加機能: 🚜 戦車（搭乗して運転できる重装甲ビークル）（js/tank.js・2026-06-21）

- **新規モジュール `js/tank.js`**（index.html では agents.js の後・ui.js の前に読み込み）。グローバル `inTank`(搭乗中か)・`tank`(現在の1台)・`tankShells[]`。`BLOCKS.TANK=110`(isTool)・ホットバー右端・アイコンは textures.js `paintTile('tank')`(オリーブドラブの横向きシルエット)。
- **操作**: TANKアイテムを右クリ/タップ→`tankItemAction`(搭乗中=砲撃 / 近く<8mに戦車=再搭乗 / それ以外=`spawnTank`して`boardTank`)。搭乗中は **WASD走行**(`updateTankDriving`)・**マウスで砲塔/砲身**(視線方向へ追従)・**左右クリック/Cで主砲**(`fireTankShell`・`TANK_FIRE_CD=0.8s`・押しっぱ連射はmain.jsが処理)・**Fで乗降**(`tankToggle`)。降車ボタン `#btn-exit-tank`(PCクリック＋モバイルタップ・input.jsで配線)。
- **モデル**: `makeTank` が THREE.Group を +Z前方で組む(履帯/車体/前面傾斜/砲塔group/砲身pivot/キューポラ)。`syncTankModel` で `group.rotation.y=π/2-hullYaw`(agents と同じ規約)、砲塔 `turretGroup.rotation.y=hullYaw-turretYaw`、砲身 `barrelPivot.rotation.x=-pitch`。
- **走行物理**: 接地は agents と同じ「下のブロックにスナップ」(`_tankGravityStep`・`below=floor(y)-1`)。前進は2段壁で停止・1段は乗り越え。`TANK_ACCEL/BRAKE/MAX_FWD12/MAX_REV6/TURN_RATE1.5`。**チェイスカメラ**=砲塔の後方上空(カメラ水平方向の後ろ`TANK_CAM_DIST8`・高さ`+3.4`)＋地面めり込み防止クランプ。`tankAimDir`=YXZオイラーから視線方向を直接算出(matrixWorldのフレーム遅延回避)。
- **砲撃**: 砲弾(`tankShells`)は updateRockets を手本にDDA着弾判定→`explode(x,y,z,false,TANK_SHELL_R)`。`TANK_SHELL_R=6`(**整数必須**)。`tank_fire`音(audio.js・新規)＋マズルフラッシュ＋画面揺れ＋砲身リコイル。**クレーターは通常爆発と同じ＝editsByChunk経由でlive↔reload一致**(別機構不要)。
- **兵を轢く**: 走行中(`|speed|>0.6`)に `killAgentsInRadius(t.x,t.y+0.4,t.z,2.3)`＝AI陣営戦の戦場に突っ込んで履帯で潰せる。砲弾の explode も兵を巻き込む。
- **★explode の小数radiusトラップ(この機能で発見)**: `explode` の掘削Y下限 `bottomY=Math.floor(cy)-min(radius,30)` は **radiusが小数だと非整数**になり、整数indexのdata走査とズレて**サイレントに何も削れない**(砲弾が当たってもクレーターが出ない)。核/メガが整数radiusなのはこのため。戦車砲も整数6にして回避。**⚠ロケラン(`1+power*0.5`=小数)も同じ潜在バグの可能性＝別件で要確認**(コアexplodeは今回は触っていない=Chesterton's Fence)。
- **main.js統合**: エンティティ更新群に `updateTanks(delta)` を追加。搭乗中はマウス押しっぱで砲撃しつつ採掘/設置をガード、`updateGunAnimation` の後に**早期return**して徒歩物理をスキップ(カメラはチェイスが設定済み)。`attemptMine`/`attemptPlaceOrIgnite` も先頭で inTank ガード(モバイルのタップ発砲がここを通る)。
- **奈落ガード**: 搭乗中(`updateTankDriving`)も無人(`idleTank`)も `t.y<worldBottomY-4` で `removeTank`(遠方で地形アンロード時に落ち続けないように)。
- **selftest 29項目**: 11m(召喚/搭乗/降車＝接地＆inTankトグル)・11n(前進で位置が動く)・11o(砲弾が壁にクレーター)・11p(走行で兵を轢く)。終了時に `inTank=false`/`removeTank`/`tankShells` を必ず片付け(外側finallyにも保険)。**敵対監査workflow(5レンズ→検証)で確認**。
- **未対応(次段)**: モブ/巨大モンスター(ライフルや砲撃の的)→プロペラ機で空中戦。複数戦車・敵戦車(ボス)は将来(agents機構で陣営戦に戦車ユニットを足せる)。
