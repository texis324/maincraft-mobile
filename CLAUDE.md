...
## 追加機能: 🤖 AI破壊軍団 + 陣営戦（赤軍 vs 青軍）（js/agents.js）

召喚アイテム **SUMMONER(=105)** 右クリで「ボクセル兵の群れ」を召喚し、世界をカオスに破壊する。
さらに **陣営戦**＝赤軍(faction0)と青軍(faction1)を左右から同時召喚して中央で会戦させる。

- **性能の土台（数百体でも軽い）**: ①描画＝単一 `THREE.InstancedMesh`(agentMesh・1ドローコール)＋`setColorAt`で陣営色を per-instance tint ②思考＝ラウンドロビン(`AGENT_THINK_BUDGET=14`体/frameだけ最寄り敵を索敵・残りは惰性) ③破壊＝`setBlockData`(edit記録しない＝肥大回避)+`markDirty`で dirty に積むだけ→main.js が予算で flush ④`VIEW_DIST*16` 外/奈落のユニットは消滅。`AGENT_MAX=260`/`AGENT_WAVE=28`(赤14+青14)。実測 140体で updateAgents avg0.14ms/max0.7ms・FPS60。
- **陣営戦の挙動**: 各兵は `{faction, hp, target, atkCd, hitFlash, alive}` を持つ。索敵=思考フレームで最寄りの敵陣ユニット(`AGENT_SIGHT=64`)を `target` に。移動=毎フレーム target 方向へ向き直して前進(壁は段差登り or 掘って突破)。戦闘=射程`AGENT_ATK_RANGE=1.7`内で `AGENT_ATK_CD=0.5s` 毎に `AGENT_ATK_DMG=2` を与える(`AGENT_HP=6`＝3発で撃破)。被弾で白フラッシュ(hitFlash)＋軽いノックバック。敵が居なくなった側はそのまま地形破壊モードに戻る。**核なし＝戦闘は近接melee のみ**。engaged(白兵戦中)の兵は前進も地形掘削もしない。小爆発(`AGENT_BLAST_CHANCE`)は `!a.target`(地形破壊モード)の兵だけ＋グローバルCD。
...
- **selftest**: 11e(legion summon+update＋色バッファ検証)・11f(隣接した赤×青の近接戦闘でHP減少/撃破)。`window.__selftest()` で緑確認。
