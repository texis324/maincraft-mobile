...
- **改変の永続(editsByChunk)**: プレイヤー設置/採掘/爆破を `editsByChunk[ck]={"x,y,z":type}`(0=撤去)に記録、チャンク(再)生成時に再適用＝アンロード→再ロードで建造物/クレーターが残る。**チャンク別索引**なので生成時に全edit走査しない(大爆発で数万editでも軽い)。**未生成チャンクへの爆破は `carveUnloaded` が「自然地形なら固体だった」セルの撤去editだけ記録**＝遠方ミサイル着弾のクレーターも後で訪れると存在(チャンク生成不要でフリーズしない)。⚠**localStorage には未保存＝ページリロードで改変は消える**(worldSeedは保存＝同じ地形が戻る。旧来も同じ挙動)。将来 editsByChunk を localStorage 保存予定(サイズ上限要)。
...
- **未対応(意図的スコープ外)**: editsBy​ChunkのlocalStorage保存(リロード跨ぎ)・洞窟(3Dノイズ)・地中貫通核・水中核・Web Worker化(移動中の生成カクつきの本丸・現状はメインスレッド予算制)・爆発の水平範囲は radius のまま(既存挙動)。
