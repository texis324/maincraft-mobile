...
- **selftest**: 11e(legion summon+update＋色バッファ検証)・11f(離れた赤×青が**石の足場上で撃ち合い**HP減少/撃破＋曳光弾発生。⚠自然地形だと起伏で埋まる/LOS不通になるので平らな足場を`setBlockData`で敷く)・11g(プレイヤーの爆発で兵が死ぬ)。`window.__selftest()` で緑確認(現在20項目)。
