...
let _nearEvents = [];          // プレイヤー近傍の爆発イベント（深い穴の底ロード判定用・列×全イベントを避ける）
...
function columnLoadRange(ccx, ccz) {
    const cxw = ccx * CS + (CS >> 1), czw = ccz * CS + (CS >> 1);
...
function streamWorld(px, py, pz) {
    ...
    // 近傍の爆発イベントだけ抽出（columnLoadRange が深い穴の底まで読むのに使う・全列×全イベントを回避）
    const reach = (VIEW_DIST + 2) * CS;
    _nearEvents.length = 0;
    for (let i = 0; i < explosionEvents.length; i++) {
        const ev = explosionEvents[i];
        if (Math.abs(ev.cx - px) < reach + ev.R && Math.abs(ev.cz - pz) < reach + ev.R) _nearEvents.push(ev);
    }
